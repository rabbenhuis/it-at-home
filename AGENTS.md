...
- Per-guest firewall rules are derived from the host's `role` via `deploy/firewall.tf` (`local.firewall_rules`, deny-by-default with a DROP tail; mgmt SSH/ICMP base from VLANs 120/122 is prepended). The rule set replaces the guest's whole `.fw` file, so it must be complete. Only registered roles get rules; others stay ACCEPT. The adguard role's port-53 rules use the cluster-level ipset `+adguard-dns-sources` (`deploy/ipsets.tf`, built from `adguard_dns_vlans`) so DNS is restricted per node.
- **DNS views**: internal record visibility is per-VLAN. Unbound runs one instance per view (ports 5353–5357; `unbound_views` in the unbound role holds the records); AdGuard routes VLANs to views via per-client upstreams (`adguard_view_clients`). Internal records must live in the unbound views, **not** in AdGuard rewrites (rewrites are global and bypass the views). Internal names live under the private-only `.internal` TLD (`abbenhuis.internal`); each view serves its records from a per-view `auth-zone` zonefile (`for-downstream: no`, `for-upstream: yes`) so CNAMEs (e.g. `file-server.abbenhuis.internal`) expand into fully-resolved answers — unbound's `local-data` CNAMEs are served bare and break Windows resolvers, and an auth-zone on the public `abbenhuis.net` fails DNSSEC (the zone is signed). The zone is declared `domain-insecure`. The public `abbenhuis.net` is recursion-only. VLAN search domains (`dns_zone` in `modules/cluster-data`) are `.internal` too.
...
- **TEMP pve2: not installed yet.** The pve2 pieces of `deploy/main.tf` (the `pm_api_token_secret_pve2` variable, the `proxmox.pve2` provider, the `pve2_hosts` local, and the `deploy_pve2` module) are commented out, and `deploy-hosts.sh` only loads pve1 creds (`load_pm_creds pve1`). To re-enable when pve2 is up: uncomment those four blocks and restore `load_pm_creds --all` + the `TF_VAR_pm_api_token_secret_pve2` assert in `deploy-hosts.sh`. All current hosts are pve1, so nothing else changes.
...
- **`scripts/pve-hosts.sh [pve1|pve2] [--bootstrap] [--check] [--user NAME]`** runs the pve role. Normal SSH user is `ansible`; `--bootstrap` uses `root` + `PVE_ROOT_PASSWORD` (sshpass); `--user sysadm1n` (key `~/.ssh/id_rsa`, sudo password via `PVE_SUDO_PASSWORD` or `--ask-become-pass`) is for the first run on an existing host that has no `ansible` user yet. `--check` is a dry-run that never applies. If `PVE_SYSADM1N_PASSWORD` is set (env or bws secret), the role sets the OS `sysadm1n` password (enables PVE GUI login as `sysadm1n@pam`; PAM users authenticate against the OS account — `ansible` stays passwordless).
- **`scripts/bootstrap-pve.sh pve1|pve2 [--register-only]`** bootstraps a fresh install: via `pveum` it ensures user `terraform@pve` and creates the API token `infra` (`--privsep 0`), stores the token id + per-node secret in bws, then auto-runs `pve-hosts.sh --bootstrap` (which creates the `Terraform-Infra` role, ACLs, OS users + hardening via the pve role). `--register-only` stores an already-existing token in bws without SSH (for pve1).
- **PVE access-control model** (managed by the pve role / bootstrap via `pveum`, **not** Terraform): `terraform@pve` → `Terraform-Infra` on `/`; `sysadm1n@pam` → `Administrator` on `/`; `ansible@pam` → `PVEAuditor` on `/`. PAM users authenticate against the OS users the role creates. Keep the `pveum` role/user/acl tasks idempotent (add-if-missing; never remove existing ACLs). The superseded `TerraformRole` / `Terraform-Deployer` roles are deleted by the pve role once unreferenced.
...
- **Keep `MaxSessions` high enough for ansible pipelining** — with `pipelining=True` ansible opens one SSH session per module; `MaxSessions 2` rejects the 3rd concurrent session during bursts of copy/template tasks, truncating in-flight transfers to 0-byte files baked into templates. Use 10 (sshd default). Symptom: a template whose tail files (firstboot, netplan override, msmtprc, aide conf) are intermittently 0 bytes while earlier files are intact.
- Netplan VMs (Debian 13/systemd 257): a static-IPv4 cloud-init config makes networkd auto-start a DHCPv6 client that fails with `ENOENT`, leaving eth0 `failed` and the static IP unapplied at boot. The vm role writes `/etc/netplan/99-ipv4-only.yaml` (`accept-ra:false`, `link-local:[]`, `dhcp6:false`) and `firstboot.sh` regenerates it as a safety net. **PVE hosts are unaffected**: the pve role never runs `base`, has no firstboot/netplan, and removes `/etc/netplan/99-ipv4-only.yaml` defensively (netplan is absent on PVE - it uses ifupdown2).
